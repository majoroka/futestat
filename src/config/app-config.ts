...
export interface AppConfig {
  ...
  competitionStandingsEnabled: boolean;
  competitionStandingsMaxAgeHours: number;
  competitionStandingsOutputDir: string;
...
export function loadAppConfig(argv = process.argv.slice(2)): AppConfig {
  ...
  const competitionStandingsMaxAgeHours = readInteger(
    undefined,
    process.env.FUTESTAT_COMPETITION_STANDINGS_MAX_AGE_HOURS,
    6,
    "competitionStandingsMaxAgeHours",
  );

  if (competitionStandingsMaxAgeHours < 1) {
    throw new Error(
      `competitionStandingsMaxAgeHours must be >= 1. Received ${competitionStandingsMaxAgeHours}.`,
    );
  }
  ...
  return {
    ...
    competitionStandingsEnabled: readBoolean(
      undefined,
      process.env.FUTESTAT_COMPETITION_STANDINGS_ENABLED,
      true,
    ),
    competitionStandingsMaxAgeHours,
    competitionStandingsOutputDir: path.resolve(
      process.env.FUTESTAT_COMPETITION_STANDINGS_OUTPUT_DIR ??
        path.join(cli.outputDir ?? process.env.FUTESTAT_OUTPUT_DIR ?? "data/fixtures", "standings"),
    ),
